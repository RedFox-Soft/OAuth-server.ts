...
export const PRICING_FAQ: readonly QuestionAnswer[] = [
	...
	},
	{
		...
		answer:
			'FoxAuth can run in production today, self-hosted. The current release is 0.4.0, and a 0.x version means the HTTP surface and the admin API may still change between minor releases. Read the changelog before upgrading and re-run the setup script afterwards. The protocol endpoints follow the specs, so a client written for FoxAuth is in practice written against the RFC and should move to another server with little change.'
	},
	{
