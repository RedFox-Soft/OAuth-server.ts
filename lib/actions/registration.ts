	...
	async function validateInitialAccessToken(ctx, next) {
		...
		switch (initialAccessToken && typeof initialAccessToken) {
			...
			case 'string': {
				...
				ctx.assert(
					valid,
					new InvalidToken('invalid initial access token value')
				);
				break;
			}
			default:
	...
	async function registrationResponse(ctx) {
		...
		if (
			...
			(typeof issueRegistrationAccessToken === 'function' &&
				issueRegistrationAccessToken(ctx))
		) {
...
export const put = [
	...
	async function forbiddenFields(ctx, next) {
		...
		ctx.assert(
			!hit,
			new InvalidRequest(`request MUST NOT include the ${hit} field`)
		);
		await next();
	},
	...
	async function equalChecks(ctx, next) {
		...
		if ('client_secret' in ctx.oidc.body) {
			const clientSecretValid = constantEquals(
				typeof ctx.oidc.body.client_secret === 'string'
	...
	async function clientUpdateResponse(ctx) {
		...
		const secretRequired =
			!ctx.oidc.client.clientSecret && provider.Client.needsSecret(properties);
