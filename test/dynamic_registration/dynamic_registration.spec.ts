	...
	context('POST /reg', () => {
			...
			return this.agent
				...
				.expect((response) => {
					...
					expect(response.body)
						.to.have.property('grant_types')
						.and.eql(['authorization_code']);
					expect(response.body)
						.to.have.property('response_types')
						.and.eql(['code']);
					expect(response.body).to.have.property(
		...
		context('when issueRegistrationAccessToken is false', () => {
			...
			it('omits issuing a registration access token and does not return registration_client_uri', function () {
				return this.agent
					...
					.expect(201)
					.expect((response) => {
						expect(response.body).not.to.contain.keys(
							'registration_client_uri',
							'registration_access_token'
						);
					});
			...
			it('populates ctx.oidc.entities', function (done) {
				this.assertOnce((ctx) => {
					expect(ctx.oidc.entities).not.to.have.property(
						'RegistrationAccessToken'
					);
				}, done);
		...
		context(
			...
			() => {
				...
				it('populates ctx.oidc.entities', function (done) {
					this.assertOnce((ctx) => {
						expect(ctx.oidc.entities).to.have.property(
							'RegistrationAccessToken'
						);
					}, done);
			...
			describe('fix string one', () => {
				...
				it('rejects calls with bad access token', function () {
					return this.agent
						...
						.expect(
							this.failWith(401, 'invalid_token', 'invalid token provided')
						);
				});
			});
			...
			describe('using a model', () => {
				...
				it('rejects calls with not found access token', function () {
					return this.agent
						...
						.auth('Loremipsumdolorsitametconsecteturadipisicingelitsed', {
							type: 'bearer'
						})
						.expect(
							this.failWith(401, 'invalid_token', 'invalid token provided')
	...
	context('GET /reg/:clientId', () => {
		...
		it('invalidates registration_access_token if used on the wrong client', function () {
			...
			return this.agent
				...
				.expect(() => {
					expect(spy.calledOnce).to.be.true;
					expect(spy.firstCall.args[0].constructor.name).to.equal(
						'RegistrationAccessToken'
					);
