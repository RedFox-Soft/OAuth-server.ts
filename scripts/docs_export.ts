 ...
 * backend off the connection strings alone — not off `NODE_ENV` — so all three are deleted here to
 * force the in-memory stores regardless of what the shell or a dotenv file (Bun loads `.env.local`
 * before any script line runs) provides; otherwise the export's `value` fields would carry a live
 * database's settings instead of the shipped defaults, and a machine with both strings set would
 * not run this at all, since setting both is refused. `NODE_ENV=test` is still
