...
import { ReRenderError } from '../helpers/re_render_errors.ts';
...
export default function getErrorHandler(provider, eventName) {
	return async function errorHandler(ctx, next) {
		const {
			features: {
				deviceFlow: { charset, userCodeInputSource }
			}
		...
		try {
			...
			if (err.expose && !(err instanceof ReRenderError)) {
				debug(
					'path=%s method=%s error=%o detail=%s',
					ctx.path,
					ctx.method,
					out,
					err.error_detail
			...
			if (ctx.oidc?.session && userInputRoutes.has(ctx.oidc.route)) {
				...
				await userCodeInputSource(
					ctx,
					formHtml.input(
						ctx.oidc.urlFor('code_verification'),
						secret,
						err.userCode,
						charset
					),
