...
describe('grant_type=authorization_code', () => {
	...
	context(
		'with real tokens (1/3) - more than one redirect_uris registered',
		() => {
			beforeEach(function () {
				return this.login();
			});
			...
			it('populates ctx.oidc.entities (w/ offline_access)', function (done) {
				...
				this.TestAdapter.for('Grant').syncUpdate(
					this.getSession().authorizations.client.grantId,
					{
						scope: 'openid offline_access'
					}
				);
				this.TestAdapter.for('AuthorizationCode').syncUpdate(
					this.getTokenJti(this.ac),
					{
						scope: 'openid offline_access'
					}
				);
			...
			it('consumes the code', function () {
				return this.agent
					...
					.expect(() => {
						expect(this.code)
							.to.have.property('consumed')
							.and.be.most(epochTime());
					})
					.expect(200);
			});
	...
	context(
		'with real tokens (2/3) - one redirect_uri registered with allowOmittingSingleRegisteredRedirectUri=false',
		() => {
	...
	context(
		'with real tokens (3/3) - one redirect_uri registered with allowOmittingSingleRegisteredRedirectUri=true',
		() => {
			beforeEach(function () {
				i(
					this.provider
			...
			it('populates ctx.oidc.entities (w/ offline_access)', function (done) {
				...
				this.TestAdapter.for('Grant').syncUpdate(
					this.getSession().authorizations.client2.grantId,
					{
						scope: 'openid offline_access'
					}
				);
