...
describe('BaseToken', () => {
	...
	it('assigns returned consumed prop', async function () {
		...
		expect(await this.provider.RefreshToken.find(token)).to.have.property(
			'consumed',
			true
		);
	});
	...
	it('rethrows adapter#find errors from session bound tokens looking up the session', async function () {
		...
		sinon
			.stub(this.TestAdapter.for('Session'), 'findByUid')
			.callsFake(async () => {
				throw adapterThrow;
			});
	...
	it('rethrows adapter#findByUserCode errors (Device Code)', async function () {
		const adapterThrow = new Error('adapter throw!');
		sinon
			.stub(this.TestAdapter.for('DeviceCode'), 'findByUserCode')
			.callsFake(async () => {
				throw adapterThrow;
			});
		return assert.rejects(
			this.provider.DeviceCode.findByUserCode('123-456-789').then(() => {
