...
- [[cookie-path-scoping]] — `(name, domain, path)` identifies a cookie, so `remove()` — which omits `Path` and lets the browser default it to the request's directory — clears a different cookie than the one the browser keeps sending.
