...
describe('grant_type=client_credentials w/ resourceIndicators', () => {
	...
	it('only supports a single resource indicator', function () {
		return this.agent
			...
			.auth('client', 'secret')
			.send(
				'grant_type=client_credentials&scope=api:read&resource=urn:wl:opaque:default&resource=urn:wl:opaque:explicit'
			)
			.type('form')
			.expect(400)
	...
	it('validates each resource to be a valid URI individually', function () {
		return this.agent
			...
			.auth('client', 'secret')
			.send(
				'grant_type=client_credentials&scope=api:read&resource=urn:wl:opaque:default&resource=invalid'
			)
			.type('form')
			.expect(400)
			.expect(/invalid_target/)
